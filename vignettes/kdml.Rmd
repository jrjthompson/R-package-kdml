...
output:
  ...
  html_document:
    df_print: paged
vignette: "%\\VignetteEngine{knitr::knitr} %\\VignetteIndexEntry{absorber package}
...
library(stats)
library(MASS)
# library(kss) rename once we know the file
library(ggplot2)
```
...
The package \(\texttt{kss}\) is a package which calculates the pairwise distances between mixed-type observations consisting of numeric (continuous), factor (nominal), and ordered factor (ordinal) variables. This kernel metric learning methodology learns the bandwidths associated with each kernel function for each variable type and returns a distance matrix that can be utilized in any distance-based clustering algorithm.

We define a kernel similarity between two data points $\mathbf{x}_{i}$ and $\mathbf{x}_{j}$ two different way based on two papers. From Ghashti, J. S. and Thompson, J. R. J. (2023), the kernel distance summation \(\texttt{kdsum}\) similarity is given by 

\begin{equation}\label{eq:kdsumsim}
s_{\text{kdsum}}(\mathbf{x}_{i}, \mathbf{x}_{j} \vert \boldsymbol{\lambda}) = \prod_{k=1}^{p_c} \frac{1}{\lambda_k} K\left( \frac{x_{i,k} - x_{j,k}}{\lambda_k}\right)  + \sum_{k=p_c+1}^{p_c + p_u} L(x_{i,k},x_{j,k},\lambda_k) + \sum_{k = p_c+p_u+1}^p l(x_{i,k},x_{j,k},\lambda_k),
\end{equation}
...
These two distance calculations can be called in \(\texttt{R}\) with the package \(\texttt{kss}\) using functions \(\texttt{kdsum}\) for Equation (\ref{eq:kdsumsim}) and \(\texttt{dkss}\) for Equation (\ref{eq:dksssim}), both of which are used in the Equation (\ref{eq:kerndist}) for pairwise distance calculations.
...
You can install the released version of \textsf{kss} from [Github](https://github.com/jrjthompson/R-package-kml) with:
...
\end{equation}
  ...
  where $s_{\boldsymbol{\lambda}}(\cdot)$ is either of the similarity functions from Equations (\ref{eq:kdsumsim}) and (\ref{eq:dksssim}). This bandwidth optimization technique, called maximum-similarity cross-validation (MSCV) and be invoked directly within the distance calculation by setting the argument \(\texttt{bw = "mscv"}\) in the functions \(\texttt{kdsum}\) and \(\texttt{dkss}\). 
...
```{r, echo = TRUE, eval = FALSE}
# KDSUM distance (Equation 1 and 3)
dis_kdsum <- kdsum(df = df)
...
```{r, echo = TRUE, eval = FALSE}
# KDSUM distance (Equation 1 and 3)
dis_kdsum_np <- kdsum(df = df, bw = "np")
...
```{r, echo = TRUE, eval = FALSE}
dis_kdsum_custom_kernels <- dkss(df = df, bw = "mscv", cFUN = "c_epanechnikov",
                                 uFUN = "u_aitken", oFUN = "o_habbema")
```

If users require only the bandwidths specified from the MSCV formula in Equation (\ref{eq:mscv}), and not the pairwise distance matrix obtained from \(\texttt{kdsum}\) or \(\texttt{dkss}\), they may do so with the following function calls:

```{r, echo = TRUE, eval = FALSE}
# MSCV bandwidth specification using the similarity function in Equation (1)
mscv_kdsum(df, nstart = NULL, ckernel = "c_gaussian", 
...
mscv_dkss(df, nstart = NULL, ckernel = "c_gaussian", 
          ukernel = "u_aitken", okernel = "o_wangvanryzin", verbose = TRUE) 
```


Arguments for the \(\texttt{kdsum}\) and \(\texttt{dkss}\) functions:
