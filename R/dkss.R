dkss <- function(df, bw = "mscv", cFUN = "c_gaussian", uFUN = "u_aitken", oFUN = "o_wangvanryzin", stan = TRUE, verbose = FALSE) {
  ...
  n_con <- length(con_cols)
  n_fac <- length(fac_cols)
  n_ord <- length(ord_cols)
  con_ind <- if(n_con > 0) 1:n_con else numeric(0)
  fac_ind <- if(n_fac > 0) (n_con + 1):(n_con + n_fac) else numeric(0)
  ord_ind <- if(n_ord > 0) (n_con + n_fac + 1):(n_con + n_fac + n_ord) else numeric(0)
  df <- data.matrix(df_ordered)
  ...
  kernel_functions <- list(
    c_gaussian = function(A, B, bws, C) {sum((2 * pi)^(-1/2) * exp(-((A - B) / bws)^2 / 2))},
    c_uniform = function(A, B, bws, C) {sum(sapply(1:length(A), function(i) ifelse(abs((A[i] - B[i]) / bws[i]) <= 1, 1/2, 0)))},
    c_epanechnikov = function(A, B, bws, C) {sum(ifelse(abs((A - B) / bws) <= 1, (3/4) * (1 - ((A - B) / bws)^2), 0))},
    c_triangle = function(A, B, bws, C) {sum(ifelse(abs((A - B) / bws) <= 1, (1 - abs((A - B) / bws)), 0))},
    c_biweight = function(A, B, bws, C) {sum(ifelse(abs((A - B) / bws) <= 1, (15/32) * (3 - ((A - B) / bws)^2)^2, 0))},
    c_triweight = function(A, B, bws, C) {sum(ifelse(abs((A - B) / bws) <= 1, (35/32) * (1 - ((A - B) / bws)^2)^3, 0))},
    c_tricube = function(A, B, bws, C) {sum(ifelse(abs((A - B) / bws) <= 1, (70/81) * (1 - abs((A - B) / bws)^3)^3, 0))},
    c_cosine = function(A, B, bws, C) {sum(ifelse(abs((A - B) / bws) <= 1, (pi/4) * cos((pi/2) * ((A - B) / bws)), 0))},
    c_logistic = function(A, B, bws, C) {sum(1 / (exp((A - B) / bws) + 2 + exp(-((A - B) / bws))))},
    c_sigmoid = function(A, B, bws, C) {sum(2 / (pi * (exp((A - B) / bws) + exp(-((A - B) / bws)))))},
    c_silverman = function(A, B, bws, C) {sum(1/bws*0.5 * exp(-abs((A-B)/bws) / sqrt(2)) * sin(abs((A-B)/bws) / sqrt(2) + pi/4))},
    u_aitchisonaitken = function(A, B, bws, C) {sum(ifelse(A == B, 1 - bws, bws / (length(unique(C)) - 1)))},
    u_aitken = function(A, B, bws, C) {sum(ifelse(A == B, 1, bws))},
    o_wangvanryzin = function(A, B, bws, C) {sum(ifelse(A == B, 1 - bws, (1/2) * (1 - bws) * (bws^abs(A - B))))},
    o_aitchisonaitken = function(A, B, bws, C) {sum(ifelse(A == B, 1, bws^(abs(A - B))))},
    o_aitken = function(A, B, bws, C) {sum(ifelse(A == B, bws, (1 - bws) / (2^abs(A - B))))},
    o_habbema = function(A, B, bws, C) {sum(bws^((abs(A - B))^2))},
    o_liracine = function(A, B, bws, C) {sum(ifelse(A == B, 1, bws^abs(A-B)))}
  )  
  kernel.distance.sum <- function(con_ind, fac_ind, ord_ind, cFUN, uFUN, oFUN, A, B, bws, df) {
    ...
    compute_distance <- function(FUN, ind) {
      ...
      sum(FUN(A[ind], B[ind], bws[ind], df[, ind, drop=FALSE]) + 
            FUN(B[ind], B[ind], bws[ind], df[, ind, drop=FALSE]) - 
            FUN(A[ind], B[ind], bws[ind], df[, ind, drop=FALSE]) - 
            FUN(B[ind], A[ind], bws[ind], df[, ind, drop=FALSE]))
    }
    d_c <- if (length(con_ind) > 0) compute_distance(get_function(cFUN, "cFUN"), con_ind) else 0
    d_u <- if (length(fac_ind) > 0) compute_distance(get_function(uFUN, "uFUN"), fac_ind) else 0
    d_o <- if (length(ord_ind) > 0) compute_distance(get_function(oFUN, "oFUN"), ord_ind) else 0
    return(d_c + d_u + d_o)
  }
